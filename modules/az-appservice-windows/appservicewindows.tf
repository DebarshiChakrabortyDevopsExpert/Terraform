resource "azurerm_windows_web_app" "windows_web_app" {
  ...
  https_only                 = try(var.windows_web_app["https_only"], true)
  app_settings               = merge({ "APPINSIGHTS_INSTRUMENTATIONKEY" = data.azurerm_application_insights.exiting_application_insights.instrumentation_key }, try(var.windows_web_app.app_settings, null))
  client_certificate_enabled = try(var.windows_web_app.client_certificate_enabled, null)
  client_certificate_mode    = try(var.windows_web_app.client_certificate_mode, null)
  virtual_network_subnet_id  = try(var.windows_web_app.virtual_network_subnet_id,null)
  tags                       = try(var.windows_web_app.tags, null)

  dynamic "identity" {
    for_each = try(var.windows_web_app.identity, null) != null ? [var.windows_web_app.identity] : []
    content {
  ...
  dynamic "site_config" {
    for_each = try(var.windows_web_app.site_config, null) != null ? [var.windows_web_app.site_config] : []
  ...
  dynamic "connection_string" {
    for_each = try(var.windows_web_app.connection_string, null) != null ? [var.windows_web_app.connection_string] : []
  ...
  dynamic "auth_settings" {
    for_each = try(var.windows_web_app.auth_settings, null) != null ? [var.windows_web_app.auth_settings] : []
