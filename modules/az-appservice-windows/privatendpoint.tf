...
resource "azurerm_private_endpoint" "private_endpoint_webapp" {
  ...
  dynamic "private_dns_zone_group" {
    ...
    content {
      ...
      private_dns_zone_ids = flatten([for ids, value in try(private_dns_zone_group.value.dnszones, null) : "/subscriptions/xxxxxxxxxxxxxxxxxxxxxxxxxxx/resourceGroups/${value.dns_zone_rg_name}/providers/Microsoft.Network/privateDnsZones/${value.dns_zone_name}"])
    }
  }
}
